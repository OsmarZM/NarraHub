...
Correção: trocado o uso de `activeNav()` como sinal de "já processado" por uma chave própria (`lastSyncedRouteKey`, formato `${navId}:${universeId}`) atualizada explicitamente por `selectNav()`/`returnToLibrary()`/`openSettings()`/`setWorkspaceNavigation()` no início de cada um — antes de qualquer `await` — e comparada em `restoreRoute()`. Teste de fronteira novo (`restoreRoute() não usa activeNav()...`) proíbe a regressão de volta pra essa comparação. Validado via `ng serve` com o mesmo universo fake: navegação direta por URL para `/history` e `/timeline` agora mostra só o conteúdo da seção certa; navegação por clique em todas as seções (Escrita → Entidades → Conexões → Timeline → Planejamento → Histórico) confirmada sem duplicação nem tela em branco. `npm run test:architecture` (22/22).
