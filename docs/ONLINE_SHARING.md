...
6. A chave fica no fragmento `#k=` da URL e não é enviada nas requisições HTTP.
7. O navegador descriptografa o workspace localmente.
8. Anotações e propostas são cifradas com a mesma chave e enviadas como envelopes opacos.
9. O aplicativo consulta novas contribuições a cada 2,5 segundos, descriptografa e persiste na fila SQLite.
10. Ao aprovar uma edição, o aplicativo valida alvo e campo, atualiza o registro local e grava o evento em `change_log`.
