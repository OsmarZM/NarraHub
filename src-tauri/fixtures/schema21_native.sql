-- Fixture anônima e determinística de um banco **criado nativamente no schema 21**.
--
...
--   * duas divergências abertas, com as duas revisões preservadas e nenhuma escolhida —
--     uma edição contra edição, e uma EXCLUSÃO contra edição, que é a forma que só as
--     colunas de operação do schema 18 conseguem representar sem ambiguidade;
--   * um tombstone com **coordenadas causais** — origem e sequência da exclusão, que é o
--     que permite provar que um peer a atravessou;
--   * vetores de peers: o que cada aparelho nos disse que já viu. É a evidência que autoriza
--     a poda, e sem ela a coleta só poderia ser adivinhada;
--   * um dispositivo que saiu por **abandono**, com `exit_reason` preenchido;
--   * uma divergência com as **operações dos dois lados** registradas — sem elas, uma
--     exclusão concorrente com uma edição chega à tela como `local_rev = ''`, e a interface
--     não consegue distinguir "manter exclusão" de "não sei o que aconteceu".
--
...
INSERT INTO sync_tombstones
    (aggregate_type, aggregate_id, deleted_rev, deleted_at, origin_device_id, origin_seq) VALUES
  ('character', 'fx16-c9', 'fx16-rev-c9-b', '2026-08-11 08:30:00', 'fx16-dev-velho', 2);

-- ── Vetores de peers ────────────────────────────────────────────────────────
-- O que cada aparelho nos disse que já viu. O Notebook confirmou a exclusão
-- (seq 2 do tablet antigo); o Android **não** — e é por isso que o tombstone
-- acima não é coletável, mesmo o aparelho de origem já estando aposentado.
INSERT INTO sync_peer_vectors (peer_device_id, origin_device_id, last_seq_confirmed) VALUES
  ('fx16-dev-note',  'fx16-dev-velho', 2),
  ('fx16-dev-note',  'fx16-dev-desk',  2),
  ('fx16-dev-droid', 'fx16-dev-velho', 1),
  ('fx16-dev-droid', 'fx16-dev-desk',  2);

-- ── Uma saída por abandono ──────────────────────────────────────────────────
-- O tablet antigo foi abandonado: quebrou e não liga mais. Ele deixa de contar
-- para a retenção, e a identidade dele não pode voltar ao conjunto.
UPDATE sync_devices
   SET exit_reason = 'abandoned'
 WHERE device_id = 'fx16-dev-velho';

-- ── Divergência aberta ──────────────────────────────────────────────────────
INSERT INTO sync_divergences
    (id, aggregate_type, aggregate_id, base_rev, local_rev, remote_rev, remote_event_id,
     detected_at, resolved_at, resolution, local_operation, remote_operation) VALUES
  ('fx16-div-1', 'character', 'fx16-c1', 'fx16-rev-c1-a', 'fx16-rev-c1-b', 'fx16-rev-c1-x',
   'fx16-e-201', '2026-08-27 21:00:00', '', '', 'upsert', 'upsert'),
  -- A forma que só o schema 18 torna representável: de um lado uma EXCLUSÃO,
  -- do outro uma edição feita a partir da mesma base. Antes das colunas de
  -- operação, este caso chegava à tela com `local_rev = ''` e ficava
  -- indistinguível de "não sabemos o que tem aqui" — e as duas leituras pedem
  -- botões opostos ao escritor. Aqui `local_rev` é a revisão da própria
  -- exclusão, porque exclusão é revisão causal e não ausência de uma.
  ('fx18-div-2', 'chapter', 'fx16-c9', 'fx16-rev-c9-a', 'fx16-rev-c9-del', 'fx16-rev-c9-y',
   'fx16-e-202', '2026-09-02 14:30:00', '', '', 'delete', 'upsert');

-- ─────────────────────────────────────────────────────────────────────────────
-- Schema 20: pendencias de migracao de asset (ADR 0010)
-- ─────────────────────────────────────────────────────────────────────────────
--
-- Duas formas que so existem a partir do 20, e que nenhum banco migrado
-- produz igual: a pendencia aberta e a ja resolvida. A segunda existe para
-- provar que o bootstrap conta so as abertas -- foi exatamente esse o defeito
-- da captura da etapa 12, que contava divergencia resolvida como impedimento.
--
-- `row_id` nao tem FK de proposito: a mesma tabela atende dez superficies em
-- dez tabelas diferentes.
INSERT INTO blob_migration_issues
    (id, surface, table_name, row_id, side, reason, detail, detected_at, resolved_at) VALUES
    ('issue-0001', 1, 'attachments', 'anexo-0001', 'data_url',
     'legacy_unrecognized', 'valor inline nao e data URL reconhecida',
     '2026-09-10 12:00:00', ''),
    ('issue-0002', 9, 'sync_conflicts', 'conflito-0001', 'remote_value',
     'document_unparseable', 'documento do lado remoto nao abre como JSON',
     '2026-09-10 12:00:00', ''),
    ('issue-0003', 3, 'entities', 'entidade-0001', 'image',
     'legacy_unrecognized', 'pendencia ja resolvida pelo escritor',
     '2026-09-10 12:00:00', '2026-09-10 13:30:00');

-- ─────────────────────────────────────────────────────────────────────────────
-- Schema 21: tipo de divergencia (NH-079, etapa B1)
-- ─────────────────────────────────────────────────────────────────────────────
--
-- A forma que so existe a partir do 21: a exclusao de um pai que chegou de
-- outro aparelho e NAO rodou, porque a cascata apagaria um descendente vivo
-- que a origem da exclusao nao conhecia. O capitulo continua corrente
-- (`local_rev` = a revisao que estava valendo, que e tambem a base da exclusao
-- remota) e o anexo continua existindo. As duas divergencias acima ficam com o
-- DEFAULT 'concurrent', que e o que um banco migrado tambem teria.
INSERT INTO sync_divergences
    (id, aggregate_type, aggregate_id, base_rev, local_rev, remote_rev, remote_event_id,
     detected_at, resolved_at, resolution, local_operation, remote_operation, kind) VALUES
  ('fx21-div-3', 'chapter', 'fx21-c-pai', 'fx21-rev-pai-a', 'fx21-rev-pai-a', 'fx21-rev-pai-del',
   'fx21-e-301', '2026-09-15 10:00:00', '', '', 'upsert', 'delete', 'parent_deletion_blocked');
